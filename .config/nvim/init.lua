...
local function set_autocmds()
  -- Format on save (ONLY real file buffers, ONLY when oxfmt or efm is
  -- attached; oxfmt wins when both are attached — it only attaches in
  -- projects with oxfmt config, where prettier via efm would be wrong)
...
local setup_snacks = function()
  ...
  snacks.setup({
    ...
    picker = { enabled = true, ui_select = true, layout = { preset = "telescope" } },
  })
end
setup_snacks()
...
-- jsonls markdown-escapes hover text (spaces become &nbsp;, punctuation gets
-- backslashed); vscode renders the markdown, neovim shows it raw — undo the
-- escaping, but only when the &nbsp; signature is present so other servers'
-- hovers stay untouched
local orig_to_markdown = vim.lsp.util.convert_input_to_markdown_lines
---@diagnostic disable-next-line: duplicate-set-field
vim.lsp.util.convert_input_to_markdown_lines = function(input, contents)
  local lines = orig_to_markdown(input, contents)
  local escaped = false
...
local function lsp_on_attach(ev)
  ...
  local format_buf = function()
    local name = #vim.lsp.get_clients({ bufnr = bufnr, name = "oxfmt" }) > 0 and "oxfmt" or "efm"
    vim.lsp.buf.format({
      bufnr = bufnr,
      filter = function(c)
        return c.name == name
...
-- hover/completion/validation from $schema; formatting stays with efm/fixjson
...
vim.lsp.enable({
  ...
  -- projects that use them and coexist with eslint/prettier elsewhere
  "oxlint",
  "oxfmt",
  "gopls",
  -- "clangd",
