...
local function check_nvim_version()
  ...
  if version.major > required.major or (version.major == required.major and version.minor >= required.minor) then
    health.ok("Neovim " .. version.major .. "." .. version.minor .. "." .. version.patch)
    return true
  else
    health.error("Neovim " .. version.major .. "." .. version.minor .. " (need >= 0.10)", { "Upgrade Neovim to 0.10+" })
    return false
  end
end
